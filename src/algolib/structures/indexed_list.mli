...
val ( @:: ) : 'a -> 'a t -> 'a t
(** [x @:: lst] is [cons x lst]. Right associative. *)
...
val elem : int -> 'a t -> 'a
(** [elem i lst] retrieves element with index [i] in indexed list [lst].

    @raise Invalid_index if index is negative or exceeds list length. *)

val ( *! ) : 'a t -> int -> 'a
(** [lst *! i] is [elem i lst]. Left associative. *)

val update : int -> 'a -> 'a t -> 'a t
(** [update i x lst] sets [x] as element with index [i] in indexed list [lst].
