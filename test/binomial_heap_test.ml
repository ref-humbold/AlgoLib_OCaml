...
let rec to_list heap =
  if IntBH.is_empty heap then [] else IntBH.peek heap :: to_list (IntBH.pop heap)
...
let is_empty__when_empty__then_true =
  "is_empty When empty Then true" >:: fun _ ->
    (* given *)
    let test_object = IntBH.empty in
    (* when *)
    let result = IntBH.is_empty test_object in
    (* then *)
...
let is_empty__when_not_empty__then_false =
  "is_empty When not empty Then false" >:: fun _ ->
    (* given *)
    let test_object = IntBH.of_list numbers in
    (* when *)
    let result = IntBH.is_empty test_object in
    (* then *)
...
let peek__when_empty__then_empty_heap =
  "peek When empty Then empty heap" >:: fun _ ->
    ...
    let test_object = IntBH.empty in
    (* when *)
    let exec () = IntBH.peek test_object in
    (* then *)
    assert_raises IntBH.Empty_heap exec

let peek__when_single_element__then_this_element =
  "peek When single element Then this element" >:: fun _ ->
    (* given *)
    let element = 19 in
    let test_object = IntBH.of_seq @@ Seq.return element in
    (* when *)
    let result = IntBH.peek test_object in
    (* then *)
...
let push__when_empty__then_added =
  "push When empty Then added" >:: fun _ ->
    ...
    let test_object = IntBH.empty in
    (* when *)
    let result = IntBH.push element test_object in
    (* then *)
    assert_equal ~printer:string_of_int element (IntBH.peek result)
...
let push__when_new_element__then_added =
  "push When new element Then added" >:: fun _ ->
    ...
    let test_object = IntBH.of_list numbers in
    (* when *)
    let result = IntBH.push 46 test_object in
    (* then *)
    assert_equal ~printer:string_of_int minimum (IntBH.peek result)
...
let push__when_new_element_is_greater_than_minimum__then_new_minimum =
  "push When new element is greater than minimum Then new minimum" >:: fun _ ->
    (* given *)
    let test_object = IntBH.of_list numbers and element = minimum - 3 in
    (* when *)
    let result = IntBH.push element test_object in
    (* then *)
    assert_equal ~printer:string_of_int element (IntBH.peek result)
...
let pop__when_empty__then_empty_heap =
  "pop When empty Then empty heap" >:: fun _ ->
    ...
    let test_object = IntBH.empty in
    (* when *)
    let exec () = IntBH.pop test_object in
    (* then *)
    assert_raises IntBH.Empty_heap exec
...
let pop__when_single_element__then_this_element_removed =
  "pop When single element Then this element removed" >:: fun _ ->
    (* given *)
    let test_object = IntBH.of_seq @@ Seq.return 19 in
    (* when *)
    let result = IntBH.pop test_object in
    (* then *)
    Assert.Bool.assert_true @@ IntBH.is_empty result
...
let pop__when_multiple_elements__then_minimal_element_removed =
  "pop When multiple elements Then minimal element removed" >:: fun _ ->
    ...
    let result = IntBH.pop test_object in
    (* then *)
    Assert.assert_not_equal ~printer:string_of_int minimum (IntBH.peek result)

let pop__when_multiple_calls__then_sorted_ascending =
  "pop When multiple calls Then sorted ascending" >:: fun _ ->
    (* given *)
    let test_object = IntBH.of_list numbers and expected = List.sort compare numbers in
    (* when *)
...
let merge__when_empty_and_not_empty__then_same_as_other =
  "merge When empty and not empty Then same as other" >:: fun _ ->
    (* given *)
    let test_object = IntBH.empty and other = IntBH.of_list numbers in
    (* when *)
    let result = IntBH.merge test_object other in
    (* then *)
    assert_equal ~printer:string_of_int (List.length @@ to_list other) (List.length @@ to_list result) ;
    assert_equal ~printer:string_of_int (IntBH.peek other) (IntBH.peek result)
...
let merge__when_not_empty_and_empty__then_no_changes =
  "merge When not empty and empty Then no changes" >:: fun _ ->
    ...
    let test_object = IntBH.of_list numbers in
    (* when *)
    let result = IntBH.merge test_object IntBH.empty in
    (* then *)
...
let merge__when_other_has_greater_minimum__then_new_minimum =
  "merge When other has greater minimum Then new minimum" >:: fun _ ->
    ...
    let test_object = IntBH.of_list numbers
    and other = IntBH.of_list [new_minimum; minimum + 5; minimum + 13; minimum + 20] in
    (* when *)
    let result = IntBH.merge test_object other in
    (* then *)
    assert_equal
    ...
    assert_equal ~printer:string_of_int new_minimum (IntBH.peek result)

let merge__when_other_has_less_minimum__then_minimum_remains =
  "merge When other has less minimum Then minimum remains" >:: fun _ ->
    (* given *)
    let test_object = IntBH.of_list numbers
    and other = IntBH.of_list [minimum + 5; minimum + 13; minimum + 20] in
    (* when *)
    let result = IntBH.merge test_object other in
    (* then *)
    assert_equal
...
let merge__when_shared_inner_heap__then_changed_only_merging_heap =
  "merge When shared inner heap Then changed only merging heap" >:: fun _ ->
    (* given *)
    let test_object = IntBH.empty
    and first = IntBH.of_list [10; 20]
    and second = IntBH.of_list [4; 8] in
    (* when *)
    let result1 = IntBH.merge test_object first in
    let result2 = IntBH.merge result1 second in
    (* then *)
    assert_equal ~printer:string_of_int 10 (IntBH.peek result1) ;
    assert_equal ~printer:(Printers.list string_of_int) (to_list result1) [10; 20] ;
    assert_equal ~printer:string_of_int 4 (IntBH.peek result2) ;
    assert_equal ~printer:(Printers.list string_of_int) (to_list result2) [4; 8; 10; 20] ;
    Assert.Bool.assert_true @@ IntBH.is_empty test_object ;
