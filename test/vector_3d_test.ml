...
let between__then_vector_from_begin_to_end =
  ...
  >:: fun _ ->
    (* when *)
    let result = between (Point_3d.Point3D (2.4, 7.8, -10.3)) (Point_3d.Point3D (-1.5, 13.2, 15.8)) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (-3.9, 5.4, 26.1)) result

let coordinates__then_triple_of_coordinates =
  "coordinates Then triple of coordinates"
  >:: fun _ ->
    (* when *)
    let result = coordinates @@ Vector3D (5.0, -19.0, 14.2) in
    (* then *)
    assert_equal
...
let coordinates_list__then_list_of_coordinates =
  ...
  >:: fun _ ->
    (* when *)
    let result = coordinates_list @@ Vector3D (5.0, -19.0, 14.2) in
    (* then *)
...
let length__then_length_of_vector =
  ...
  >:: fun _ ->
    (* when *)
    let result = length @@ Vector3D (18.0, -6.0, 13.0) in
    (* then *)
...
let dot__then_scalar_product =
  ...
  >:: fun _ ->
    (* when *)
    let result = dot (Vector3D (1.5, -4.0, -3.5)) (Vector3D (9.0, -2.5, 8.5)) in
    (* then *)
...
let dot__when_orthogonal__then_zero =
  ...
  >:: fun _ ->
    (* when *)
    let result = dot (Vector3D (1.0, 0.0, 1.0)) (Vector3D (0.0, -2.0, 0.0)) in
    (* then *)
...
let cross__then_cross_product =
  ...
  >:: fun _ ->
    (* when *)
    let result = cross (Vector3D (1.5, -4.0, -3.5)) (Vector3D (9.0, -2.5, 8.5)) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (-42.75, -44.25, 32.25)) result

let cross__when_parallel__then_zero =
  "cross When parallel Then zero"
  >:: fun _ ->
    (* when *)
    let result = cross (Vector3D (3.0, 3.0, 3.0)) (Vector3D (-8.0, -8.0, -8.0)) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (0.0, 0.0, 0.0)) result

let area__then_length_of_cross_product =
  "area Then length of cross product"
  >:: fun _ ->
    (* when *)
    let result = area (Vector3D (1.5, -4.0, -3.5)) (Vector3D (9.0, -2.5, 8.5)) in
    (* then *)
...
let area__when_parallel__then_zero =
  ...
  >:: fun _ ->
    (* when *)
    let result = area (Vector3D (3.0, 3.0, 3.0)) (Vector3D (-8.0, -8.0, -8.0)) in
    (* then *)
    assert_equal ~cmp:float_cmp ~printer:string_of_float 0.0 result
...
let volume__then_scalar_triple_product =
  ...
  >:: fun _ ->
    (* when *)
    let result =
      volume (Vector3D (1.5, -4.0, -3.5)) (Vector3D (9.0, -2.5, 8.5)) (Vector3D (1.0, -1.0, 1.0))
    in
    (* then *)
...
let volume__when_parallel__then_zero =
  ...
  >:: fun _ ->
    (* when *)
    let result =
      volume (Vector3D (3.0, 3.0, 3.0)) (Vector3D (-8.0, -8.0, -8.0)) (Vector3D (2.0, -2.0, 2.0))
    in
    (* then *)
    assert_equal ~cmp:float_cmp ~printer:string_of_float 0.0 result
...
let volume__when_orthogonal__then_zero =
  ...
  >:: fun _ ->
    (* when *)
    let result =
      volume (Vector3D (3.0, 3.0, 3.0)) (Vector3D (1.0, 0.0, 1.0)) (Vector3D (0.0, -2.0, 0.0))
    in
    (* then *)
    assert_equal ~cmp:float_cmp ~printer:string_of_float 0.0 result
...
let op_tilde_minus_dollar__then_negate_each_coordinate =
  ...
  >:: fun _ ->
    (* when *)
    let result = ~-$(Vector3D (5.4, 9.0, -12.3)) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (-5.4, -9.0, 12.3)) result

let op_plus_dollar__then_add_each_coordinate =
  "op plus dollar Then add each coordinate"
  >:: fun _ ->
    (* when *)
    let result = Vector3D (5.4, 9.0, -12.3) +$ Vector3D (7.9, -8.1, 1.4) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (13.3, 0.9, -10.9)) result
...
let op_minus_dollar__then_subtract_each_coordinate =
  ...
  >:: fun _ ->
    ...
    let result = Vector3D (5.4, 9.0, -12.3) -$ Vector3D (7.9, -8.1, 1.4) in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (-2.5, 17.1, -13.7)) result

let op_asterisk_dollar__then_multiply_each_coordinate =
  "op asterisk dollar Then multiply each coordinate"
  >:: fun _ ->
    (* when *)
    let result = Vector3D (5.4, 9.0, -12.3) *$ 3.0 in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (16.2, 27.0, -36.9)) result
...
let op_asterisk_dollar__when_multiplication_by_zero__then_zero_vector =
  ...
  >:: fun _ ->
    (* when *)
    let result = Vector3D (5.4, 9.0, -12.3) *$ 0.0 in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (0.0, 0.0, 0.0)) result
...
let op_slash_dollar__then_divide_each_coordinate =
  ...
  >:: fun _ ->
    ...
    let result = Vector3D (5.4, 9.0, -12.3) /$ 3.0 in
    (* then *)
    assert_equal ~cmp:equal ~printer:print_vector (Vector3D (1.8, 3.0, -4.1)) result
...
let op_slash_dollar__when_division_by_zero__then_division_by_zero =
  ...
  >:: fun _ ->
    (* when *)
    let exec () = Vector3D (1.0, 1.0, 1.0) /$ 0.0 in
    (* then *)
